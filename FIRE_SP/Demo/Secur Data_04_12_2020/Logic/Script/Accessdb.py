...
if os.path.isfile("info.log"):
    os.remove(r''+myPath['Log']+"\Accessdb\info.log")
    logging.basicConfig(filename=r''+myPath['Log']+"\Accessdb\info.log", format='%(asctime)s %(message)s', filemode='w') 
...
try:
    ...
    ADB_data = pd.read_csv(r''+myPath['Src']+"\Access DB Secure.csv",encoding='latin1')
    #Metadata
