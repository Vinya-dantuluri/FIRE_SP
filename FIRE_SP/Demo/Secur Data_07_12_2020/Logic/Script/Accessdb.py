...
try:
    ...
    ignore_list = ignore_list['IGNORE LIST'].str.upper().values.tolist()
    def Mapping(df,dic):
        new_var =[]
    #     print(df['Customer'])
    ...
    def effectivedate(data_Active1):
        try:
            ...
            list3=['ANNUAL','MONTHLY','BI-MONTHLY','QUARTELY','WEEKLY','MONTHLY & MONITORING','MONTHLY QI ONLY','QUARTELY','MI','MONTHLY/QTLY','MONTHLY/QI','MONTHLY/WKLY']
...
#Acronyms#
    def MapDesc(data_Active):
        ...
        KMP_CL_F= (data_Active
                .set_index(['Alt Agreement','Agreement Type '+'('+'Data Pull)','Customer','Customer2','Agreement Price','Customer PO','Service Site','Description','Effective Date',
            'Original Date','Description1','Expiration Date'])['C_list']
                .apply(pd.Series)
                .stack()
                .reset_index()
                #.drop('level_4', axis=1)
    ...
    try:
        ...
        output[['Alt Agreement','Agreement Type '+'('+'Data Pull)','Customer','Agreement Price','Customer PO','Service Site',
                'Description']] = data_Active[['Inspection Quote #','Inspection Type','Legal Company Name','Price','PO#','Site Address','Fire Protection Equipment']]

        output=output[(output['Description'].str.contains('cancelled')==False) & (output['Description'].str.contains('Cancelled')== False)]

        # out_ignore=output[output['Customer'].astype(str).isin(['AXIA PROPERTY MANAGEMENT (CCC 516)'])]
        # out_ignore.to_csv('D:\FSP\FPS Client 0211\secur\Target\Accessdb\ignore_list_active.csv',index=False)
        # output=output[~output['Customer'].astype(str).isin(['AXIA PROPERTY MANAGEMENT (CCC 516)'])]

        output['newcol']=output['Customer'].str.upper()
        result=output[(output['newcol'].isin(ignore_list))]
        output = output[~(output['newcol'].isin(ignore_list))]
        result.drop('newcol',axis='columns', inplace=True)
        ...
        lis =['Original Date','Effective Date','Expiration Date']
        ...
        #ignore list#######################################
        result['Description'] = dataClean(result)
        result['Customer1'] = Mapping(result,CT_ED_Dic)
        result['Customer2']= result['Customer']
        result['Customer']=result['Customer1']
        result['Expiration Date'] = '01/01/2022'
        temp=MapDesc(result)
        ...
        for i in DateColumns:
            if (str(i).find('Date') != -1):
                if str(i) not in lis:
                    result[i]= output1[i].fillna('01/01/2021')
        ...
        result=output1[output1['Description'].str.contains('Monitoring')==False]
        result.to_csv(ignorelist_Access_A,index=False)
        ###################################################
        ...
        output['Effective Date']=pd.to_datetime(output['Effective Date'])
        output['Expiration Date']= output['Effective Date'] - pd.DateOffset(months=1)
        output['Expiration Date']= output['Effective Date'] - pd.DateOffset(days=1)
        ...
        output1 = pd.DataFrame(columns=list(Metadata['Output File1']))
        output1[['Alt Agreement','Agreement Type '+'('+'Data Pull)','Customer','Agreement Price','Customer PO','Service Site',
            'Description','Effective Date','Original Date','Description1','Expiration Date']] = temp[['Alt Agreement','Agreement Type '+'('+'Data Pull)','Customer',
                                                    'Agreement Price','Customer PO','Service Site','Cus-Desc','Effective Date','Original Date',
                                                                    'Description1','Expiration Date']]
        for i in DateColumns:
            if (str(i).find('Date') != -1):
                if str(i) not in lis:
        ...
        monitering=output1[output1['Description'].str.contains('Monitoring')==True]
        monitering.to_csv(monitoring_Active,index=False)
        #output1=output1[output1['Description'].str.contains('Monitoring')==False]
    ...
    try:
        ...
        result=output1[output1['Description'].str.contains('Monitoring')==False]
        result.to_csv(ignorelist_Access_I,index=False)
        ###################################################
        ...
        monitering=output1[output1['Description'].str.contains('Monitoring')==True]
        monitering.to_csv(monitoring_InActive,index=False)
        #output1=output1[output1['Description'].str.contains('Monitoring')==False]
